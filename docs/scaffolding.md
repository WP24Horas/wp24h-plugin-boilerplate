...
Isso mantém alinhados o cabeçalho principal, text domain, namespace PSR-4, constantes, referências internas, testes, documentação e metadados do Composer.
...
- `.git`, `vendor` e `dist` não são copiados;
- o próprio `bin/wp24h-init` é removido do plugin gerado;
...
4. política de licença e autoria;
5. namespace, slug e text domain com uma busca final;
...
O scaffolder existe para automatizar apenas transformações determinísticas e deixar decisões de produto — descrição, módulos, branding e release — explícitas para o desenvolvedor.
